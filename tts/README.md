# 本地神经语音（TTS = Kokoro int8 中英，ASR = SenseVoice）

## 能力

- **TTS**：sherpa-onnx **Kokoro**（`kokoro-int8-multi-lang-v1_1`），int8 量化，CPU 可实时。默认中文女声 `zf_xiaoxiao`（sid=47）
...
curl -X POST http://127.0.0.1:8090/v1/audio/speech \
  ...
  -d "{\"input\":\"你好，我是智枢。\",\"speaker_id\":47,\"response_format\":\"wav\"}" \
  --output tts.wav
```
