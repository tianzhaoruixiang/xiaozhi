...
|------|------|
| `config` | 只用配置的专家团 + 工作流 |
| `dynamic` | 智枢每轮动态生成专家（旧行为） |
| `hybrid` | 先识别意图：普通问题由智枢直接执行（Claude Code / 模型）；今日安排问询答复日程；办会任务用配置工作流（默认） |

意图分流：`现在几点了` → 智枢直接答时间；`今天重点事项` → 答复日程；`准备会议并通知` → 人员调度会专家团；`找推荐领域专家` → 领域专家寻访专家团；`准备线上沟通` → 线上沟通专家团；`邀请线下沟通` → 线下沟通邀约专家团。专家团/工作流留空即为智能识别。
